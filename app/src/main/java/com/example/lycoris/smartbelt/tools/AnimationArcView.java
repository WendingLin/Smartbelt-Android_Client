...
public class AnimationArcView extends View {
    //圆环颜色
    ...
    public AnimationArcView(Context context, int score, int mPage, int max) {
        super(context);
        setValue(score, mPage);
        this.max=max;
    }

    public AnimationArcView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public AnimationArcView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawPoint(Canvas canvas) {
        ...
        initPaint();
        canvas.drawCircle((float) (9.0f * tb + 8.5f * tb * Math.sin(3.6 * pointCount * Math.PI / 180 /max)),
                (float) (9.0f * tb - 8.5f * tb * Math.cos(3.6 * pointCount * Math.PI / 180 /max)), 10, paint_big_text);
